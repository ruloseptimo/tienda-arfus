...
exports.createBuyerEvent = async (req, res) => {
    try {
        const { buyer_id, event_type } = req.body; // Obtiene los datos del evento desde el cuerpo de la solicitud.
        // Crea un nuevo evento del comprador en la base de datos.  Incluye la fecha del evento.
        const newEvent = await BuyerEvent.create({ buyer_id, event_type, event_date: new Date() }); 
